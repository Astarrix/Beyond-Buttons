...
#define _ANY_PACKAGE_ ((UPackage*)-1)
...
const FClassDefinition & IKMGC_ScriptParser::GetClassInfo(const FString &Keyword) {
	...
	if (NPK.RemoveFromStart(TEXT("A"),ESearchCase::CaseSensitive)) {
		if (UClass*Class=FindObject<UClass>(_ANY_PACKAGE_,*NPK,true)) {
		...
		if (UScriptStruct*Struct=FindObject<UScriptStruct>(_ANY_PACKAGE_,*NPK,true)) {
			DB->RegisterStructReflection(Struct);
			return DB->ClassDefinitions.FindChecked(Struct->GetPrefixCPP()+NPK);
		}
	} else if (UEnum*Enum=FindObject<UEnum>(_ANY_PACKAGE_,*Keyword,true)) {
...
const FSlateBrush* IKMGC_ScriptParser::GetTypeIcon(const FString &Notation) {
	...
	//
	if (Notation.Contains(TEXT("OBJ|"),ESearchCase::CaseSensitive)) {Brush=FAppStyle::GetBrush(TEXT("CodeView.ClassIcon"));}
	if (Notation.Contains(TEXT("SET|"),ESearchCase::CaseSensitive)) {Brush=FAppStyle::GetBrush(TEXT("Kismet.VariableList.SetTypeIcon"));}
	if (Notation.Contains(TEXT("ARR|"),ESearchCase::CaseSensitive)) {Brush=FAppStyle::GetBrush(TEXT("Kismet.VariableList.ArrayTypeIcon"));}
	if (Notation.Contains(TEXT("MAP|"),ESearchCase::CaseSensitive)) {Brush=FAppStyle::GetBrush(TEXT("Kismet.VariableList.MapValueTypeIcon"));}
	//
	return Brush;
}
