...
void SKMGC_TextEditorWidget::Construct(const FArguments &InArgs) {
	...
	TypeWidth = FontMeasure->Measure(TEXT("W"),FKMGC_NodeStyle::Get().Get()->GetWidgetStyle<FTextBlockStyle>("KMGC.CodeBlockStyle").Font,FKMGC_NodeStyle::Get().Get()->GetWidgetStyle<FTextBlockStyle>("KMGC.CodeBlockStyle").Font.Size).X;
	LineHeight = FontMeasure->Measure(TEXT("W"),FKMGC_NodeStyle::Get().Get()->GetWidgetStyle<FTextBlockStyle>("KMGC.CodeBlockStyle").Font,FKMGC_NodeStyle::Get().Get()->GetWidgetStyle<FTextBlockStyle>("KMGC.CodeBlockStyle").Font.Size).Y;
	//
...
int32 SKMGC_TextEditorWidget::OnPaint(const FPaintArgs &Args, const FGeometry &Geometry, const FSlateRect &CullingRect, FSlateWindowElementList &OutDrawElements, int32 LayerID, const FWidgetStyle &WidgetStyle, bool ParentEnabled) const {
	...
	if (!bIsReadOnly.Get()) {
		for (int32 L=0; L<=(LineCount); L++) {
			FSlateDrawElement::MakeBox(
				...
				Geometry.ToPaintGeometry(FVector2D(0,(LineHeight*L)-EditableTextLayout->GetScrollOffset().Y),FSlateLayoutTransform(FVector2D(AllotedWidth,LineHeight))),
	...
	if (!bIsReadOnly.Get()) {
		FSlateDrawElement::MakeBox(
			OutDrawElements,LayerID,
			Geometry.ToPaintGeometry(FVector2D(0,(LineHeight*CursorLocation.GetLineIndex())-EditableTextLayout->GetScrollOffset().Y),FSlateLayoutTransform(FVector2D(AllotedWidth,LineHeight))),
		...
		LayerID++;
		//
		if (false) { //(HasSuggestion()) { // Disabled until figure out what is the graphics glitch
			...
			FSlateDrawElement::MakeBox(
				OutDrawElements,LayerID,
				Geometry.ToPaintGeometry(BorderPos,FSlateLayoutTransform(BorderSize)),
			...
			FSlateDrawElement::MakeBox(
				OutDrawElements,LayerID,
				Geometry.ToPaintGeometry(BoxPos,FSlateLayoutTransform(BoxSize)),
			...
			for (int32 I=SuggestDrawID; I<(SuggestDrawID+MAX_SUGGESTIONS); ++I) {
				if (SuggestionResults.IsValidIndex(I)) {
					...
					FSlateDrawElement::MakeBox(
						OutDrawElements,LayerID,
						Geometry.ToPaintGeometry(Position,FSlateLayoutTransform(FVector2D(LineFraction,LineFraction))),
					...
					FSlateDrawElement::MakeText(
						OutDrawElements,LayerID,
						Geometry.ToPaintGeometry(TextPos,FSlateLayoutTransform(FVector2D(BoxSize.X,LineHeight))),
			...
			if (SuggestPicked>INDEX_NONE) {
				FSlateDrawElement::MakeBox(
					OutDrawElements,LayerID,
					Geometry.ToPaintGeometry(BoxPos,FSlateLayoutTransform(FVector2D(BoxSize.X,LineHeight))),
					FKMGC_NodeStyle::Get().Get()->GetBrush("KMGC.Lines"),
		...
		if (!KeywordInfo.IsEmpty()) {
			...
			FSlateDrawElement::MakeBox(
				OutDrawElements,LayerID,
				Geometry.ToPaintGeometry(FVector2D(BoxOffset.X,((CursorLocation.GetLineIndex()*LineHeight)-EditableTextLayout->GetScrollOffset().Y)+(LineHeight*(SuggestPicked+1))),FSlateLayoutTransform(BorderSize)),
				FAppStyle::GetBrush("ToolPanel.DarkGroupBorder"),
				DrawEffects,FLinearColor::White
			);
			//
			FSlateDrawElement::MakeText(
				OutDrawElements,LayerID,
				Geometry.ToPaintGeometry(FVector2D(BoxOffset.X+4,((CursorLocation.GetLineIndex()*LineHeight)-EditableTextLayout->GetScrollOffset().Y)+(LineHeight*(SuggestPicked+1.1f))),FSlateLayoutTransform(FontSize)),
				KeywordInfo,
...
const FSlateBrush* SKMGC_TextEditorWidget::GetSuggestionIcon(const FString &Keyword) const {
	...
	if (HasSuggestion()) {
		const FKeywordDefinition &KeyInfo = IKMGC_ScriptParser::GetKeywordInfo(Keyword);
		if (KeyInfo!=IKMGC_ScriptParser::NOKeywordInfo) {Brush=FAppStyle::GetBrush(TEXT("Kismet.AllClasses.VariableIcon")); return Brush;}
		//
		const FClassDefinition &ClassInfo = IKMGC_ScriptParser::GetClassPointerInfo(Keyword);
		if (ClassInfo!=IKMGC_ScriptParser::NOClassInfo) {Brush=FAppStyle::GetBrush(TEXT("CodeView.ClassIcon")); return Brush;}
		//
		if (ScriptObject.IsValid()) {
			const FFunctionDefinition &FunInfo = IKMGC_ScriptParser::GetFunctionInfo(ScriptObject->GetRuntimeScriptClass(),Keyword);
			if (FunInfo!=IKMGC_ScriptParser::NOFunctionInfo) {Brush=FAppStyle::GetBrush(TEXT("Kismet.AllClasses.FunctionIcon")); return Brush;}
			//
			const FPropertyDefinition &PropInfo = IKMGC_ScriptParser::GetPropertyInfo(ScriptObject->GetRuntimeScriptClass(),Keyword);
			if (PropInfo!=IKMGC_ScriptParser::NOPropertyInfo) {
				switch (PropInfo.StackOf) {
					case EStack::Set: {Brush=FAppStyle::GetBrush(TEXT("Kismet.VariableList.SetTypeIcon"));} break;
					case EStack::Array: {Brush=FAppStyle::GetBrush(TEXT("Kismet.VariableList.ArrayTypeIcon"));} break;
					case EStack::Map: {Brush=FAppStyle::GetBrush(TEXT("Kismet.VariableList.MapValueTypeIcon"));} break;
				default: break;}
			}
		}
	}
