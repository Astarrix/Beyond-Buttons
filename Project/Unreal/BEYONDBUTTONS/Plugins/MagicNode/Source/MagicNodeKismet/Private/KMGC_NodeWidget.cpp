					...
					[
						...
						[
							...
							[
								SNew(SBorder)
								.VAlign(VAlign_Fill).HAlign(HAlign_Fill)
								.BorderImage(FAppStyle::GetBrush("Sequencer.Thumbnail.SectionHandle"))
								[
									SNew(SBorder)
									.VAlign(VAlign_Fill).HAlign(HAlign_Fill)
									.BorderImage(FAppStyle::GetBrush("ToolPanel.DarkGroupBorder"))
									[
										...
										[
											...
											[
												...
												[
													SNew(SImage)
													.Image(FAppStyle::Get().GetBrush(TEXT("Symbols.SearchGlass")))
												]
											]
										]
										+SVerticalBox::Slot().Padding(5)
...
const FSlateBrush* SKMGC_MagicNodeWidget::GetCompilerIcon() const {
	if (!HasScript()) {return FAppStyle::GetBrush(TEXT("Kismet.Status.Good"));}
	UPackage* Package = GetScriptObject()->GetOutermost();
	//
	if (Package->IsDirty() && GraphNode->ErrorType >= EMessageSeverity::Info) {
		return FAppStyle::GetBrush(TEXT("Kismet.Status.Unknown"));
	} else if (GraphNode->ErrorType == EMessageSeverity::Warning) {
		return FAppStyle::GetBrush(TEXT("Kismet.Status.Warning"));
	} else if (GraphNode->ErrorType <= EMessageSeverity::Error) {
		return FAppStyle::GetBrush(TEXT("Kismet.Status.Error"));
